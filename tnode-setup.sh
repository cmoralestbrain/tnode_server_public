...
def handle_drive_wiki_sync(token: dict, params: dict) -> dict:
    ...
    for entry in files:
        ...
        try:
            ...
            continue
        safe = re.sub(r"[^\w.\- ]", "_", name).strip().replace(" ", "_") or fid
        if not re.search(r"\.(md|txt)$", safe, re.I):
            safe += ".md"
        tmp = Path(tempfile.gettempdir()) / f"dws_{safe}"
        try:
            tmp.write_text(f"# {name}\n\n{text}\n")
            _run_openclaw("wiki", "ingest", str(tmp), timeout=90)
        finally:
            try:
                tmp.unlink()
            except Exception:  # noqa: BLE001
